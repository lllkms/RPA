...
async def generate_qr(data: str = Form(...)):
    ...
    # 바이너리 데이터로 변환
    img_bate_arr = BytesIO()
    img.save(img_byte_arr, format='PNG')
    img_byte_arr = img_bate_arr.getvalue()
    
    return StreamingResponse(BytesIO(img_bate_arr), media_type="image/png")
...
def loginDB(userId, userPassword):
    ...
    if result:
        ...
        print("Login fails. Invalid username or passwrod.")
        return False
...
def login_form(userid: str = Form(...), userpassword: str = Form(...)):
    ...
    if result == True :
        return {"mag" : f"{userid}님 반갑습니다."}
    else :
        return {"mag" : f"로그인에 실패했습니다.."}
